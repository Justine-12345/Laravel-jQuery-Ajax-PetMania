$('#adoptNav').click(function(){
 ...
 $('#adoptableRow').html('');
		...
		type: "GET",
		    ...
		    success: function(data){
		    	$.each(data.data, function( index, value ) {
				  ...
				  if (!value.adopters[0] && !value.diseases[0]) {
				  	console.log(value);
				  		...
				  		card += '<div class="caption"><h3>'+value.animal_name+' <i class="fas fa-check-circle" id="badge" style="color:#3dc404; font-size:20px" title="Healthy Badge"></i> </h3><ul style="list-style:none;" id="adoptableAnimalDetail">';
					...
					$('#adoptableRow').hide().append(card).show(400);
				});
		    },
		    error: function(data){}
	});
	...
	$('#adoptableSearch').autocomplete({
		 source:function(request, response){
		 	var term = $('#adoptableSearch').val();
		 	console.log(term);
        $.ajax({
            url:'/api/search/'+term,
            type:'GET',
    ...
    select:function(event, ui){
        ...
        $.ajax({
            type:"GET",
            url:"/api/search/click/"+ui.item.value,
            dataType:"json",
            success:function(data){
                $('#adoptableRow').html('');
                console.log(data);
                ...
                // $.each(data.data, function( index, value ) {
		    		var card ='';
				  if (!data.data.adopters[0] && !data.data.diseases[0]) {
				  		card += '<div class="col-md-3" ><div class="thumbnail"><img data-id="'+data.id+'" class="adoptableAnimalPicture" src="'+data.data.animal_picture+'" class="card-img-top" alt="Animal Picture" style="height: 30vh; width: 100%; object-fit: cover;cursor:pointer;">';
				  		card += '<div class="caption"><h3>'+data.data.animal_name+' <i class="fas fa-check-circle" id="badge" style="color:#3dc404; font-size:20px" title="Healthy Badge"></i> </h3><ul style="list-style:none;" id="adoptableAnimalDetail">';
						card += '<li>Type: '+data.data.category.category_name+'</li><li>Breed: '+data.data.animal_breed+'</li><li>Age: '+data.data.animal_age+'</li><li>Gender: '+data.data.animal_age+'</li><li>Health: '+data.data.animal_health+'</li><hr><li>Rescuer: '+data.data.rescuers[0].rescuer_fname+' '+data.data.rescuers[0].rescuer_lname+'</li><hr>';
						}
					$('#adoptableRow').hide().append(card).show(400);
				// });
	...
	$('#adoptableSearch').keyup(function(){
		...
		$.ajax({
		    ...
		    success: function(data){
		    	...
		    	if (data.diseases[0]) {
		   		...
		   		$('#showAnimalFrontContent #s_veterinarian_id').html(data.veterenarians[0].id);
		   		}
		   		...
		   		$('#showAnimalFrontContent #s_rescuer_id').html(data.rescuers[0].id);
		   		}
...
//Comment Store
	$('#animalCommentSubmit').on('click',function(e){
		e.preventDefault();
	            ...
	            success: function(data) {  	 
	            	$('#commentform').get(0).reset();
	            	 var created_at = data.created_at;
		   			 var date = created_at.split('T'); 
	               	 var commenthtml = '<div class="col-lg-12" style="background-color: #5bc0de; height: 100px; border-radius: 10px; padding:24px; color:#04426e; margin-bottom: 24px"><div class="row"><div class="col-lg">';
		   				commenthtml += 'Commented By: '+ data.comment_name +' | <small>'+date[0]+'</small></div></div><br><div class="row"><div class="col-lg" style="margin-left:24px; margin-top:10px;margin-bottom:50px">';
		   				commenthtml += ''+data.comment_content+'</div></div></div><br>';
		   				$('#commentContainer').hide().append(commenthtml).fadeIn(400);
	            },
	...
	//Adoption Store
	$('#adoptionRequestformSubmit').on('click',function(e){
		e.preventDefault();
	    var data = $('#adoptionRequestform')[0];
	    let formData = new FormData($('#adoptionRequestform')[0]);
	  	 for (var pair of formData.entries()) {
	    console.log(pair[0]+ ', ' + pair[1]); 
	    }
	    $.ajax({
	            ...
	            dataType: "json",
	            success: function(data) {  	 
	            	$('#adoptionRequestform').get(0).reset();
	            	console.log(data);
	            	$("#adoptionRequestModal").modal("hide");
	            	$('#adoptBtnForm').attr('disabled',true);
		    		$('#adoptBtnForm').html('Adopted Already');
		    		$('#notYetAdopted').html('Adoption Request Sent');
